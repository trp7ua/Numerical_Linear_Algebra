...
from SparseStorage import MatrixtoCSR, SparseGet
from Operations import MMultiply, SparseDot
import time

def Jacobi(A, b, error_tol=1e-10):
    t1 = time.time()
    (m, n) = np.shape(A)
    x = np.zeros_like(b)
...
def SparseJacobi(Acsr, b, error_tol=1e-10):
    t1 = time.time()
    m = Acsr['m']
    x = np.zeros_like(b)
    x_prev = np.zeros_like(b)
...
if __name__ == "__main__":
    ...
    xsp, errorsp, itrsp = SparseJacobi(Acsr, b, 1e-10)
